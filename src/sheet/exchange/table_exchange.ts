...
class TableExchange implements SetUp {
  private _startCol: LetterInteger;
  private _startRow: number;
  private _code: string;
  private _food: string;
  private _grams: number;
  private _foodExchange: string;
  private _gramsExchange: number;
  private _homeUnit: string;
  private _cellCode: cellObject;
  private _cellFood: cellObject;
  private _cellGrams: cellObject;
  private _cellFoodExchange: cellObject;
  private _cellGramsExchange: cellObject;
  private _cellHomeUnit: cellObject;

  constructor(startCol: LetterInteger, startRow: number) {
   ...
   */
  static fromJSON(json: any): TableExchange {
  ...
  private clearCells(): void {
    ...
    if (!sheet) throw new Error(`Sheet ${SHEET_EXCHANGES} not found`);
   ...
   */
  public dropdown(cell: string): void {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_EXCHANGES);
    if (!sheet) throw new Error(`Sheet ${SHEET_EXCHANGES} not found`);
  ...
  clean(): void {
    ...
    Utils.deleteValidations(
      [sheet.getRange(this._startRow, this._startCol).getA1Notation()],
      SHEET_EXCHANGES
    );
  }
}
